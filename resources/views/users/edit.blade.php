    ...
    <form action="{{ route('users.update', $user->id) }}" method="POST" class="grid gap-4 w-full">
        ...
        <div class="flex gap-6 justify-stretch w-full">
            ...
            <div class="grid gap-4 w-[50%]">
                <div class="w-full">
                    ...
                    <input type="text" name="name" class="w-full focus:outline-none p-2 border-2 border-clr-light-gray/40 rounded-md mt-2" placeholder="Name" value="{{ $user->name }}">
                </div>
                <div class="w-full">
                    <label class="text-clr-dark" for="username">Username</label>
                    <input type="text" name="username" class="w-full focus:outline-none p-2 border-2 border-clr-light-gray/40 rounded-md mt-2" placeholder="Username" value="{{ $user->username }}">
                </div>
                <div class="w-full">
                    <label class="text-clr-dark" for="lastname1">Last Name 1</label>
                    <input type="text" name="lastname1" class="w-full focus:outline-none p-2 border-2 border-clr-light-gray/40 rounded-md mt-2" placeholder="Last Name 1" value="{{ $user->lastname1 }}">
                </div>
                <div class="w-full">
                    <label class="text-clr-dark" for="lastname2">Last Name 2</label>
                    <input type="text" name="lastname2" class="w-full focus:outline-none p-2 border-2 border-clr-light-gray/40 rounded-md mt-2" placeholder="Last Name 2" value="{{ $user->lastname2 }}">
                </div>
            </div>
        </div>
        <div class="flex gap-4 justify-center">
            <div class="w-full">
                ...
                <input type="email" name="email" class="w-full focus:outline-none p-2 border-2 border-clr-light-gray/40 rounded-md mt-2" placeholder="Email" value="{{ $user->email }}">
            </div>
            <div class="w-full">
                <label class="text-clr-dark" for="password">Password</label>
                <input type="password" name="password" class="w-full focus:outline-none p-2 border-2 border-clr-light-gray/40 rounded-md mt-2" placeholder="Password" value="{{ old('password') }}">
            ...
            <div class="w-full">
                ...
                <select name="users_types_id" id="user_type" class="w-full focus:outline-none p-2 border-2 border-clr-light-gray/40 rounded-md mt-2">
                    @foreach ($users as $userType)
                    <option value="{{ $userType->id }}" {{ $userType->id == $user->users_types_id ? 'selected' : '' }}>
                        {{ $userType->name }}
                    </option>
                    @endforeach
                </select>
        ...
        <button type="submit" class="w-full p-2 bg-[#4958a3] text-white rounded-sm hover:brightness-[.85] duration-150">Save Changes</button>
    </form>
</div>
@endsection