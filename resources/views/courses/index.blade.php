...
@section('content')
    ...
    @endif
    <form action="{{ route('courses.search') }}" method="GET" class="p-4 dark:bg-gray-800 rounded-lg">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-6">
                ...
                <div>
                    <label for="acronyms"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300">Acronyms:</label>
                    <input type="text" id="acronyms" name="acronyms"
                        class="mt-1 block w-48 px-3 py-2 rounded-md shadow-sm border-gray-300 focus:border-clr-blue focus:ring focus:ring-clr-blue focus:ring-opacity-50 transition duration-150 ease-in-out" placeholder="Search...">
                </div>
            </div>
            <button type="submit"
            ...
            <div class="flex gap-2">
                ...
                @for ($i = $start; $i <= $end; $i++)
                    <a href="{{ $courses->url($i) }}"
                        class="px-4 py-2 hover:bg-gray-300 rounded-full transition duration-300 @if ($courses->currentPage() === $i) text-white bg-clr-dark-blue cursor-not-allowed @endif">{{ $i }}</a>
                @endfor
            </div>
