...
<div class="grid max-w-[900px] gap-4 bg-white rounded-sm my-4 mx-auto p-6">
    <section>
        <a href="{{ url()->previous() }}" class="cursor-pointer text-2xl mb-2 font-semibold text-clr-dark-third inline-block">
            <</a>
                <h2 class="text-2xl mb-2 font-semibold text-clr-dark-third inline-block">New course</h2>
                <p class="text-clr-dark-gray">Complete all the data to create a new course.</p>
    </section>
    ...
    <form action="{{ route('courses.store') }}" method="POST" id="form-create" class="grid gap-4 w-full" enctype="multipart/form-data">
        @csrf
            ...
            <div class="grid gap-4 w-[50%]">
                <div class="flex gap-4 items-center">
                    <div class="w-full">
                        ...
                        <input type="text" name="name" required class="w-full focus:outline-none p-2 border-2 border-clr-light-gray/40 rounded-md mt-2" placeholder="Ej. Web design">
                    </div>
                    <div>
                        <label class="text-clr-dark" for="acronyms">Acronyms</label>
                        <input type="text" name="acronyms" required class="w-full focus:outline-none p-2 border-2 border-clr-light-gray/40 rounded-md mt-2" placeholder="Ej. WD">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" required class="w-full focus:outline-none resize-none p-2 border-2 border-clr-light-gray/40 rounded-md mt-2" placeholder="Give a brief overview of this course"></textarea>
                </div>
            </div>
        </div>
        <button type="submit" class="w-full p-2 bg-[#4958a3] text-white rounded-sm hover:brightness-[.85] duration-150">Create course</button>
    </form>
</div>
