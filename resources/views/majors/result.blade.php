...
<section class="mt-6 mb-4 text-clr-dark-blue">
    ...
    <div class="mb-4">
        <a href="{{ route('activities.index') }}" class="p-2 bg-gray-200 rounded-md text-clr-dark-gray hover:brightness-[.80] duration-100">
            Go Back
        </a>
    </div>
...
<div class="bg-white rounded-md my-4">
            ...
            <tr class="border-b border-neutral-200 dark:border-white/10">
                ...
                <td class="py-2">
                    <form action="{{ route('majors.destroy', $result->id) }}" method="POST">
                        <a class="p-2 bg-blue-100 rounded-md text-clr-blue me-2 my-2 hover:brightness-[.80] duration-100" href="{{ route('majors.show', $result->id) }}">Show</a>
                        <a class="p-2 bg-gray-200 rounded-md text-clr-dark-gray me-2 my-2 hover:brightness-[.80] duration-100" href="{{ route('majors.edit', $result->id) }}">Edit</a>
                        @csrf
                        @method('DELETE')
    ...
    <div class="p-4 flex items-center justify-between">
        ...
        <div class="flex gap-2">
            ...
            @endphp
            @for ($i = $start; $i <= $end; $i++)
                <a href="{{ $results->url($i) }}" class="px-4 py-2 hover:bg-gray-300 rounded-full transition duration-300 @if ($results->currentPage() === $i) text-white bg-clr-dark-blue cursor-not-allowed @endif">{{ $i }}</a>
            @endfor
        </div>
